...
# DataFrame
results = pd.DataFrame(columns=['I', 'T', 'K', 'pattern', 'scenario', 'prob', 'epsilon', 'chi', 'objval', 'lbound', 'iteration', 'time_sp', 'time_rmp', 
    ...
    for chi in [3]:
        for len_I in [100]:
            for pattern in ['Medium']:
                ...
                scenario_value = df_demand[(df_demand['I'] == len(I)) & (df_demand['Pattern'] == pattern)].iloc[0]['Scenario']
                eps = epsilon
                ...
                undercoverage, understaffing, perfloss, consistency, consistency_norm, undercoverage_norm, understaffing_norm, perfloss_norm, final_obj, final_lb, itr, lagrangeB, ls_sc_behav, ls_p_behavior, undercoverage_per_shift, time_sps, time_rmp, time_ip, nr_itr = column_generation_behavior(data, demand_dict, eps, Min_WD_i, Max_WD_i, time_cg_init, max_itr, output_len, chi,
                ...
                print('Values a,b', shift_undercover_naive, shift_undercover_behaviour)
